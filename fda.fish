function fda --description 'Change to a directory using fuzzy finder (including hidden directories)'
	...
	find $base -type d ^ /dev/null | fzf +m --query=$query --select-1 --exit-0 > $TMPDIR/dirs
